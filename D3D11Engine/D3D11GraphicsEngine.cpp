...
const D3DXVECTOR4 UNDERWATER_COLOR_MOD = D3DXVECTOR4(0.8f, 0.8f, 1.0f, 1.0f);
...
XRESULT D3D11GraphicsEngine::OnResize(INT2 newSize)
{
	...
	delete GBuffer0_Diffuse;
	GBuffer0_Diffuse = new RenderToTextureBuffer(Device, Resolution.x, Resolution.y, DXGI_FORMAT_R16G16B16A16_FLOAT);
...
void D3D11GraphicsEngine::SetDefaultStates()
{
	...
	Engine::GAPI->GetRendererState()->BlendStateDirty = true;
	Engine::GAPI->GetRendererState()->DepthStateDirty = true;
	Engine::GAPI->GetRendererState()->SamplerStateDirty = true;
...
XRESULT D3D11GraphicsEngine::DrawLighting(std::vector<VobLightInfo*>& lights)
{
	...
	D3DXMatrixOrthoLH(&cr.ProjectionReplacement, WorldShadowmap1->GetSizeX() * Engine::GAPI->GetRendererState()->RendererSettings.WorldShadowRangeScale, 
												 WorldShadowmap1->GetSizeX() * Engine::GAPI->GetRendererState()->RendererSettings.WorldShadowRangeScale, 1, 40000.0f);
	...
	for(std::vector<VobLightInfo*>::iterator itv = lights.begin(); itv != lights.end();itv++)
	{
		...
		if(D3DXVec3Length(&(*plcb.Pl_PositionWorld.toD3DXVECTOR3() - Engine::GAPI->GetCameraPosition())) < plcb.PL_Range)
		{
...
void D3D11GraphicsEngine::BindShaderForTexture(zCTexture* texture, bool forceAlphaTest, int zMatAlphaFunc)
{
	...
	if(zMatAlphaFunc == zMAT_ALPHA_FUNC_BLEND)
	{
		newShader = PS_Simple;
...
void D3D11GraphicsEngine::DrawDecalList(const std::vector<zCVob *>& decals, bool lighting)
{
	...
	std::vector<ParticleInstanceInfo> instances;
...
void D3D11GraphicsEngine::DrawFrameParticles(std::map<zCTexture*, std::vector<ParticleInstanceInfo>>& particles, std::map<zCTexture*, ParticleRenderInfo>& info)
{
	...
	BaseVertexBuffer* ib;
	Engine::GraphicsEngine->CreateVertexBuffer(&ib);
	ib->Init(NULL, 6 * sizeof(VERTEX_INDEX), BaseVertexBuffer::EBindFlags::B_INDEXBUFFER, BaseVertexBuffer::EUsageFlags::U_DYNAMIC, BaseVertexBuffer::CA_WRITE);
	...
	VERTEX_INDEX indices[] = {0,1,2,3,4,5};
	ib->UpdateBuffer(indices, sizeof(indices));
	...
	state.BlendStateDirty = true;
	
	state.DepthState.DepthWriteEnabled = false;
	state.DepthStateDirty = true;
	...
	state.RasterizerStateDirty = true;
